...
class DbHandler {
    ...
    public function createIncident($username,$image,$imgmp,$age, $gender, $date,$location, $location_details, $contact,$description ) {
        ...
        $id=0; 
        $api_key = $this->generateApiKey();
        $actualpath = "http://helpclick.ahsanaasim.me/v1/$image.png";
...
public function getAllIncidentComments($id) {
        $stmt = $this->conn->prepare("SELECT username,comment FROM comments_posts where post_id =? order by id desc");
...
public function getAllNoticeComments($id) {
        $stmt = $this->conn->prepare("SELECT username,comment FROM comments_notices where notice_id= ? order by id desc");
